...
def Get_Mess(url_son):
    ...
    data_key = ['导演', '编剧', '主演', '类型', '制片国家/地区', '语言', '上映日期', '片长', '又名', 'IMDb']
    ...
    # 地区国家 语言 又名 IMDb
    obj = re.compile(
        r'制片国家/地区:</span> (?P<location>.*?)<br/>.*?语言:</span> (?P<language>.*?)<br/>.*?又名:</span> (?P<rename>.*?)<br/>.*?IMDb:</span> (?P<IMDb>.*?)<br>',
        re.S)
    ...
    mess = [leader, screenwriter, video_actor, video_types, location, language, times, runtime, rename, IMDb]
    # print(mess)
...
if __name__ == '__main__':
    ...
    get_mess = Get_Mess(get_url[4])
    print(get_mess)

    header = ['rank', 'name', 'summary', 'rating_num', 'votes', 'weights', '导演', '编剧', '主演', '类型', '制片国家/地区', '语言', '上映日期', '片长', '又名', 'IMDb']
